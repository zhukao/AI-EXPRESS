...
class HOBOT_EXPORT Workflow {
  ...
  */
  ...
  virtual void Feed(spRunContext run_context,
                    ...
                    int forward_index,
                    spMessage message,
                    int64_t feed_id = FeedID::INVALID_FEED_ID) = 0;
  ...
  */
  ...
  /**
  * Feed and run the graph.
  * Feed multiple messages once, with same feed_id
  * @param run_context
  * @param feed_messages
  * @param feed_id 如果用户想使用SyncOnFeed的功能，
   *          最好保证每次Feed的feed_id单调递增。
  */
  virtual void Feed(spRunContext run_context,
  ...
  /**
   * Set input slots of module forward synchronize according to feed_id.
   *
   * The Messages in the input_slots with the same feed_id are checked
   * against this module's Condition; As a result, messages in a Forward()
   * invocation will have the same feed_id if the corresponding input slots
   * are specified in input_slots
   *
   * @param module
   * @param forward_index default to empty list
   * @param input_slots 需要同步的input_slot列表。
   *            如果设置为{ReqAll}，则表示所有的有效input_slot都需要同步。
   * @param order_by_feedid
   */
  virtual void SetSyncOnFeed(Module *module,
                             int forward_index,
                             std::vector<int> input_slots,
                             bool order_by_feedid = false) = 0;
...
class HOBOT_EXPORT  RunObserver {
  ...
  */
  virtual void OnExecTimeout(Module */*from*/,
                             int /*forward_index*/, int /*run_time*/) { }
...
class HOBOT_EXPORT Message {
 public:
  virtual ~Message();
  /**
   * id indicating which feed() invocation this Message originated from
   * -1 for invalid
   */
  int64_t feed_id{INVALID_FEED_ID};
};

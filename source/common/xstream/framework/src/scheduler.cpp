 ...
 * @brief     the scheduler of xstream framework
 * @author    chuanyi.yang
...
int Scheduler::Init(XStreamConfigPtr config,
...
int Scheduler::SetCallback(XStreamCallback callback) {
  callback_ = callback;
  return 0;
}

int Scheduler::SetCallback(XStreamCallback callback, const std::string &name) {
...
InputParamPtr Scheduler::GetConfig(const std::string &unique_name) const {
  auto iter = name2ptr_.find(unique_name);
  if (iter != name2ptr_.end()) {
...
std::string Scheduler::GetVersion(const std::string &unique_name) const {
  auto iter = name2ptr_.find(unique_name);
  if (iter != name2ptr_.end()) {
...
int64_t Scheduler::Input(InputDataPtr input, void *sync_context) {
  ...
  for (const auto &param : input->params_) {
    framework_data->method_param_[param->unique_name_] = param;
  }
...
std::vector<OutputDataPtr> Scheduler::MultipleOutput(
    FrameworkDataPtr framework_data) {
...
int Scheduler::OutputMethodResult(FrameworkDataPtr framework_data,
  ...
  RUN_FPS_PROFILER_WITH_PROFILER(profiler_,
                             readyNode->GetUniqueName() + "method output")
  if (framework_data->sync_context_ != nullptr) {
    ...
    if (node_callbacks_.end() != node_callback_i) {
      ...
      result->context_ = framework_data->context_;
      result->sequence_id_ = framework_data->sequence_id_;
      result->unique_name_ = readyNode->GetUniqueName();
      result->error_code_ = 0;
...
int Scheduler::Schedule(FrameworkDataPtr framework_data, NodePtr readyNode) {
  ...
  if (ret < 0) {
    return HOBOTXSTREAM_ERROR_EXCEED_MAX_RUNNING_COUNT;
  }
  return 0;
}
...
int Scheduler::FreeDataSlot(const FrameworkDataPtr &framework_data,
  ...
  for (auto slot : slots) {
    ...
    if (std::find(xstream_output_names_.begin(), xstream_output_names_.end(),
                  data_name) != xstream_output_names_.end()) {
      continue;
    }
...
void Scheduler::PrepareNodeBeforeToDo(const FrameworkDataPtr &framework_data,
  ...
  for (const auto &slot : out_slots) {
    if (DataState_None != framework_data->datas_state_[slot]) continue;
...
bool Scheduler::IsFrameDone(const FrameworkDataPtr &framework_data) {
  ...
  for (auto &o_name : xstream_output_names_) {
  ...
  if (framework_data->sync_context_ == nullptr) {
    for (auto single_output : output_type_names_) {
      ...
      if ((std::find(framework_data->output_type_isout_.begin(),
                     framework_data->output_type_isout_.end(), output_type) ==
...
int Scheduler::CreateNodes() {
  for (const auto &nodeName : scheduler_config_->GetNodesName()) {
    ...
    auto node = std::make_shared<xstream::Node>(nodeName);
    ...
    node->Init(std::bind(&Scheduler::Schedule, this,

...
class Callback {
 public:
  ...
  void OnCallback(xstream::OutputDataPtr output) {
    using xstream::MulSrcTestOutput;
    using xstream::MulSrcTestOutputPtr;
    auto out_test =
  ...
  int ExamResult(xstream::MethodInfo methodinfo,
    ...
    } else if (!methodinfo.is_src_ctx_dept && thread_num !=1) {
 ...
 private:
  std::vector<xstream::MulSrcTestOutputPtr> resbuffer;
};
...
int AdjustConfigFile(const std::string& inconfig,
  ...
  auto &workflow = config[xstream::kWorkflow];
  int node_size = workflow.size();
  for (int i = 0; i < node_size; ++i) {
    auto node = workflow[i];
    auto method_name = node[xstream::kMethodType].asString();
    if (method_name == "MultiSourceTest") {
      workflow[i][xstream::kThreadNum] = thread_num > 0? thread_num:1;
      break;
    }
  }
  outfile << config;
...
int DoTest(const std::string &config_template,
           ...
           const  xstream::MethodInfo &methodinfo,
           int source_num,
           int thread_count) {
...
static xstream::MethodInfo methodinfo;
TEST(MultiSourceTestMethod, BackwardCompatible1) {
  ...
  int sourcenum = 1;
  xstream::MultiSourceTestMethod::SetMethodInfo(methodinfo);

...
int BBoxFilterTest(const std::string &config) {
  ...
  if (method_config) {
    auto real_ptr = dynamic_cast<xstream::FilterParam *>(
        method_config.get());
...
TEST(ThreadModel, compatiable) {
  ThreadModelTest::BBoxFilterTest(
      "./test/configs/BBoxFilter_default.json");
}

TEST(ThreadModel, SingleThread) {
  ThreadModelTest::BBoxFilterTest(
      "./test/configs/BBoxFilter_singleThread.json");
}

TEST(ThreadModel, CrossShare0) {
  ThreadModelTest::BBoxFilterTest(
      "./test/configs/BBoxFilter_CrossShare0.json");
}

TEST(ThreadModel, CrossShare1) {
  ThreadModelTest::BBoxFilterTest(
      "./test/configs/BBoxFilter_CrossShare1.json");
}

TEST(ThreadModel, CrossShare2) {
  ThreadModelTest::BBoxFilterTest(
      "./test/configs/BBoxFilter_CrossShare2.json");
}

...
namespace xstream {
namespace CnnProc {
DEFINE_MethodCreator(lmkpose_post, LmkPosePostProcessor())

std::vector<std::vector<BaseDataPtr>>
LmkPosePostProcessor::DoProcess(
  ...
  for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    ...
    }
    {
      ...
      for (int dim_idx = 0; dim_idx < dim_size; dim_idx++) {
        ...
        auto xstream_box =
        std::static_pointer_cast<XStreamData<hobot::vision::BBox>>(
            boxes->datas_[dim_idx]);
        HandleLmkPose(mxnet_output[dim_idx],
                      xstream_box->value,
                      run_data->md_info->real_nhwc_,
                      &output);
...
void LmkPosePostProcessor::HandleLmkPose(
...
BaseDataPtr LmkPosePostProcessor::LmkPostPro(
    const std::vector<std::vector<int8_t>> &mxnet_outs,
    const hobot::vision::BBox &box,
    const std::vector<std::vector<uint32_t>> &nhwc) {
...
BaseDataPtr
LmkPosePostProcessor::PosePostPro(const std::vector<int8_t> &mxnet_outs) {
  auto pose = std::make_shared<XStreamData<hobot::vision::Pose3D>>();

...
namespace {
void GetImage(const HorizonVisionImage &image, const cv::Mat &bgr_img,
              int level = 0) {
  ...
  case kHorizonVisionPixelFormatX2PYM: {
    ...
    auto img_y_size =
        src_img->down_scale[index].height * src_img->down_scale[index].step;
    auto img_uv_size = img_y_size / 2;

    yuv420p.create(height * 3 / 2, width, CV_8UC1);
    memcpy(yuv420p.data, reinterpret_cast<uint8_t *>(y_addr), img_y_size);
    memcpy(yuv420p.data + height * width, reinterpret_cast<uint8_t *>(uv_addr),
           img_uv_size);
...
#ifdef X3
    cv::Mat yuv420p;
    // image.data is hobot::vision::PymImageFrame *
    hobot::vision::PymImageFrame *pym_img =
        reinterpret_cast<hobot::vision::PymImageFrame *>(image.data);
    int index = level * 4;
    auto height = pym_img->down_scale[index].height;
    auto width = pym_img->down_scale[index].width;
    auto y_addr = pym_img->down_scale[index].y_vaddr;
    auto uv_addr = pym_img->down_scale[index].c_vaddr;
    HOBOT_CHECK(height) << "width = " << width << ", height = " << height;
    auto img_y_size =
        pym_img->down_scale[index].height * pym_img->down_scale[index].stride;
    auto img_uv_size = img_y_size / 2;
...
XStreamImageFramePtr *ImageConversion(const HorizonVisionImage &c_img) {
  ...
  case kHorizonVisionPixelFormatNone: {
    ...
    std::vector<unsigned char> buf(c_img.data, c_img.data + c_img.data_size);
    cv_img->img = cv::imdecode(buf, cv::IMREAD_COLOR);
  ...
  case kHorizonVisionPixelFormatRawBGR: {
    auto cv_img = std::make_shared<hobot::vision::CVImageFrame>();
    cv_img->img = cv::Mat(c_img.height, c_img.width, CV_8UC3, c_img.data);
  ...
  case kHorizonVisionPixelFormatPYM: {
    ...
    HOBOT_CHECK(img_info);
    check_pyramid(img_info);
    *pym_img = *img_info;  // default assign operator
    xstream_img->value = pym_img;
    break;
  }
#endif
...
XStreamImageFramePtr *
ImageFrameConversion(const HorizonVisionImageFrame *c_img_frame) {

...
class FirstNumBest : public SnapShot {
 ...
 private:
  ...
  bool ReadyToPost(const StatePtr &trackinfo);

  int UpdateState(const ImageFramePtr &frame, uint32_t &crop_count,
                  const XStreamBBoxPtr &pbbox, const float &select_score,

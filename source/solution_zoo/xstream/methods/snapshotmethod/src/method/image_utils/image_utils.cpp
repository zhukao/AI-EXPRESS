...
ImageFramePtr ImageUtils::DoFaceCrop(const ImageFramePtr &frame,
  ...
  int s32Ret = HobotXStreamCropImageFrameWithPaddingBlack(
      frame.get(), static_cast<const int>(crop_rect.x1),
      static_cast<const int>(crop_rect.y1),
      static_cast<const int>(crop_rect.x2),
      static_cast<const int>(crop_rect.y2), output_format, &pCropBuf,
  ...
  if (bNeedScale && need_resize) {
    ...
    int cv_ret = ImageUtils::Data2CVImage(output_height, output_width,
                                          frame->pixel_format, scale_data,
                                          snap_frame->img);

    if (cv_ret != XSTREAM_SNAPSHOT_OK) {
      return nullptr;
    }
    ...
    int cv_ret = ImageUtils::Data2CVImage(height, width, frame->pixel_format,
                                          pCropBuf, snap_frame->img);
    if (cv_ret != XSTREAM_SNAPSHOT_OK) {
      return nullptr;
    }
    std::free(pCropBuf);
  ...
  if (snap_frame) {
    if (frame->pixel_format == kHorizonVisionPixelFormatX2PYM ||
        ...
        frame->pixel_format == kHorizonVisionPixelFormatX3PYM ||
        frame->pixel_format == kHorizonVisionPixelFormatX3SRC ||
        frame->pixel_format == kHorizonVisionPixelFormatPYM) {

...
std::vector<BaseDataPtr> Crop::ProcessFrame(const std::vector<BaseDataPtr> &in,
...
std::vector<BaseDataPtr> Crop::CropAndPost(const std::vector<BaseDataPtr> &in) {
  ...
  for (size_t i = 0; i < item_size; i++) {
    auto pbbox = std::static_pointer_cast<XStreamBBox>(box_list->datas_[i]);
    if (pbbox->state_ == DataState::VALID) {
      ...
      snaps_info.push_back(SnapShotInfo::GetSnapShotInfo(
          pframe->value, 0, pbbox, snapshot_config_param_.get(),
          onetrack_user_datas));

...
#include "hobotxstream/data_types/pose_3d.h"
#include "hobotxstream/data_types/quality.h"
#include "hobotxstream/data_types/snapshot_info.h"
#include "json/json.h"

struct RectInfo : xstream::BaseData {
...
class Callback {
 public:
  ...
  int WriteLog(xstream::SnapshotInfo *snapshot_info) {
    FILE *plog = nullptr;
    plog = fopen("snap_log.txt", "a+");
    fprintf(plog, "track_id: %li, frame_id: %li\n", snapshot_info->track_id_,
            snapshot_info->origin_image_frame_->frame_id_);
    fclose(plog);
    return 0;
  }

  void write_json(xstream::SnapshotInfo *snapshot_info) {
    char filename[50];
  ...
  int DumpSnap(xstream::SnapshotInfo *snapshot_info) {
    char filename[50];
    int id = static_cast<int>(snapshot_info->track_id_);
    if (id_map.find(id) == id_map.end()) return 0;

    if (access("snaps", 2) != 0) mkdir("snaps", 00700);
    ...
    if (!fwrite(snapshot_info->snap_->data_, snapshot_info->snap_->data_size_,
                1, pfile)) {
    ...
    for (const auto &data : output->datas_) {
      ...
      if (!pdata->datas_.empty()) {
        for (const auto &item : pdata->datas_) {
          ...
          //  get the item score
          auto snapshot_info =
              std::static_pointer_cast<xstream::SnapshotInfo>(item);
          ...
          std::cout << "track_id:" << snapshot_info->track_id_
                    ...
                    << " basedata:" << userdata[0]->name_ << std::endl;
        }
      }
    }
  }
...
int ConstructInput(const std::string &smart_frame,
  ...
  using xstream::Number;
  using xstream::PixelFormat;

  static std::map<unsigned, TrackState> track_map;

  unsigned id = 0, md_id = 0;
  float x0 = 0, x1 = 0, y0 = 0, y1 = 0, pose = 0, pitch = 0, yaw = 0, roll = 0,
        quality = 0, over_quality = 0;
  ...
  std::shared_ptr<ImageFrame> img_frame(
      new ImageFrame(frame_width * frame_height, nullptr));
  ...
  while (ss >> id) {
    ...
    if (md_id == 0) {  // is fase
      ...
      data->name_ =
          std::to_string(timestamp) + "_" + std::to_string(id) + "_FaceData";
      ...
      if (bbox->state_ == DataState::VALID) {
        ...
        float frontalPos_weight_ = 0.3, size_weight_ = 0.2, blur_weight_ = 0.2,
              lmk_weight_ = 0.3;
...
int main(int argc, char const *argv[]) {
  ...
  if (!img_list_path.empty()) image_list = fopen(img_list_path.data(), "r");

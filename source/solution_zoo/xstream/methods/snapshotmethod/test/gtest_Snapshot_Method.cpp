...
class XStreamSelectMethodTest : public ::testing::Test {
 public:
  XStreamSelectMethodTest() = default;
  ...
  class Callback {
   public:
    ...
    std::string GetSimpleLog() { return simple_log_; }
    ...
    void OnCallback(const xstream::OutputDataPtr &output) {
        ...
        for (const auto &item : psnap_data->datas_) {
          ...
          for (auto &snapshot_info : one_target_snapshot_info->datas_) {
            ...
            for (size_t i = 0; i < userdatas.size(); i++) {
              if (userdatas[i]->state_ == xstream::DataState::VALID) {
                log_ += " userdata_";
      ...
      if (output->datas_.size() > 1) {
        ...
        if (!psnap_data->datas_.empty()) {
          for (const auto &item : psnap_data->datas_) {
            auto snap_state =
                std::static_pointer_cast<xstream::XStreamSnapshotState>(item);
            ...
            LOGI << "snap id: " << value->id << " "
                 ...
                 << "snap box:(" << value->box.x1 << ", " << value->box.y1
...
class SnapShotRunningParam : public xstream::InputParam {
 public:
...
class SnapShotMethodParam : public xstream::InputParam {
 public:
...
TEST_F(XStreamSelectMethodTest, Basic) {
  // init the SDK
...
TEST_F(XStreamSelectMethodTest, ConfigUpdate) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
  flow->SetCallback(
...
TEST_F(XStreamSelectMethodTest, CropGray) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
  ...
  xstream::InputDataPtr select_inputdata(new xstream::InputData());
  int ret =
      ConstructInput(smart_frame, img_path, select_inputdata, img_format, true);
  EXPECT_EQ(ret, 0);
  xstream::InputDataPtr crop_inputdata(new xstream::InputData());
  ...
  ret =
      ConstructInput(smart_frame, img_path, crop_inputdata, img_format, false);
...
TEST_F(XStreamSelectMethodTest, CropBGR) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
...
TEST_F(XStreamSelectMethodTest, CropRGB) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
...
TEST_F(XStreamSelectMethodTest, CropI420) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
...
TEST_F(XStreamSelectMethodTest, VerificationConfigFile) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
  flow->SetCallback(
...
TEST_F(XStreamSelectMethodTest, VerificationConfigUpdate) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
  flow->SetCallback(
...
TEST_F(XStreamSelectMethodTest, EmptyUserdata) {
  // init the SDK
  xstream::XStreamSDK *flow = xstream::XStreamSDK::CreateSDK();
  Callback callback;
  flow->SetCallback(
  ...
  while (getline(fin, smart_frame)) {
    ...
    auto start_time = high_resolution_clock::now();
    int ret =
        ConstructInputInvalidUserdata(smart_frame, "", inputdata, "", true);
...
TEST_F(XStreamSelectMethodTest, AddTest) {
  xstream::SnapShotInfo::GenerateWithoutSnapshot(0);
  xstream::SelectSnapShotInfo s;
  hobot::vision::Points in;
  s.PointsToSnap(in);
}
